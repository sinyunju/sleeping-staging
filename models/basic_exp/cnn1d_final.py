...
if __name__ == '__main__':
    ...
    model     = CNN1D(n_channels=2, n_classes=5,
    ...
    print(f"Params: lr={LR}, dropout={DROPOUT}, "
          f"base_filters={BASE_FILTERS}, batch_size={BATCH_SIZE}")
    start = time.time()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_f1 > best_val_f1:
            ...
            torch.save(model.state_dict(),
                       os.path.join(RESULTS_PATH, 'best_cnn1d_final.pt'))
        else:
            no_improve += 1
    ...
    with open(os.path.join(RESULTS_PATH, 'cnn1d_final.json'), 'w') as f:
        json.dump(results, f, indent=4)
    print("저장 완료: cnn1d_final.json")