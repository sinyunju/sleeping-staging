...
class CNN1D(nn.Module):
    ...
    def __init__(self, n_channels=2, n_classes=5, dropout=0.3):
        super().__init__()
        self.blocks = nn.Sequential(
            ResidualBlock(n_channels, 32,  kernel_size=50, dropout=dropout),
            nn.MaxPool1d(8, 8),
            ResidualBlock(32,  64,  kernel_size=10, dropout=dropout),
            nn.MaxPool1d(4, 4),
            ResidualBlock(64,  128, kernel_size=10, dropout=dropout),
            nn.MaxPool1d(4, 4),
        )
...
if __name__ == '__main__':
    ...
    model     = CNN1D(n_channels=2, n_classes=5, dropout=0.3).to(device)
    ...
    print(f"\n학습 시작 (총 {EPOCHS} epochs, Early stopping patience={patience})")
    start = time.time()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_f1 > best_val_f1:
            ...
            torch.save(model.state_dict(),
                       os.path.join(RESULTS_PATH, 'best_cnn1d.pt'))
        else:
            no_improve += 1
    ...
    with open(os.path.join(RESULTS_PATH, 'cnn1d_results.json'), 'w') as f:
        json.dump(results, f, indent=4)
    print("결과 저장 완료: cnn1d_results.json")