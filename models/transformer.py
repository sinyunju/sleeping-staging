...
RESULTS_PATH = os.path.expanduser(
    "~/Desktop/sleeping-staging/results/metrics")
GRID_PATH = os.path.join(RESULTS_PATH, "grid_search")
...
class SleepTransformer(nn.Module):
    ...
    def __init__(self, n_channels=2, n_classes=5,
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(n_channels, 32, kernel_size=50, padding=24),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.MaxPool1d(8, 8),

            nn.Conv1d(32, 64, kernel_size=10, padding=4),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.MaxPool1d(4, 4),

            nn.Conv1d(64, d_model, kernel_size=10, padding=4),
...
def train_model(model, train_loader, val_loader, optimizer,
                criterion, scheduler, epochs=50, patience=10,
                save_path=None):
    best_val_f1 = 0
    best_epoch  = 0
    history     = []
    ...
    return best_val_f1, best_epoch, history
...
if __name__ == '__main__':
    ...
    print("\n학습 시작 (총 50 epochs, Early stopping patience=10)")
    start = time.time()

    best_val_f1, best_epoch, history = train_model(
        model, train_loader, val_loader,
        optimizer, criterion, scheduler,
        epochs=50, patience=10,
        save_path=os.path.join(RESULTS_PATH, 'best_transformer.pt')
    )

    train_time = time.time() - start
    print(f"\n학습 완료! 총 시간: {train_time:.1f}초")
    print(f"Best epoch: {best_epoch}, Best Val F1: {best_val_f1:.4f}")

    model.load_state_dict(torch.load(
        os.path.join(RESULTS_PATH, 'best_transformer.pt'),
        map_location=device))
    ...
    with open(os.path.join(RESULTS_PATH, 'transformer_results.json'), 'w') as f:
        json.dump(results, f, indent=4)
    print("결과 저장 완료: transformer_results.json")