...
CHANNEL_CONFIGS = {
    'eeg_eog':     ['EEG Fpz-Cz', 'EEG Pz-Oz', 'EOG horizontal'],
    'eeg_eog_emg': ['EEG Fpz-Cz', 'EEG Pz-Oz', 'EOG horizontal', 'EMG submental'],
}

def process_one_subject(psg_path, hyp_path, channels):
...
def process_dataset(data_path, channels, label=''):
...
for config_name, channels in CHANNEL_CONFIGS.items():
    print(f"\n{'='*50}")
    print(f"채널 조합: {config_name} {channels}")
    print(f"{'='*50}")

    print("\n[Cassette 처리 중...]")
    X_sc, y_sc, subj_sc = process_dataset(
        CASSETTE_PATH, channels, label='cassette')
    print(f"Cassette shape: {X_sc.shape}")

    X_tr, y_tr, X_val, y_val, X_te, y_te = split_dataset(
        X_sc, y_sc, subj_sc)

    np.save(os.path.join(PROCESSED_PATH,
            f'X_train_sc_{config_name}.npy'), X_tr)
    np.save(os.path.join(PROCESSED_PATH,
            f'y_train_sc_{config_name}.npy'), y_tr)
    np.save(os.path.join(PROCESSED_PATH,
            f'X_val_sc_{config_name}.npy'),   X_val)
    np.save(os.path.join(PROCESSED_PATH,
            f'y_val_sc_{config_name}.npy'),   y_val)
    np.save(os.path.join(PROCESSED_PATH,
            f'X_test_sc_{config_name}.npy'),  X_te)
    np.save(os.path.join(PROCESSED_PATH,
            f'y_test_sc_{config_name}.npy'),  y_te)
    print(f"Cassette {config_name} 저장 완료!")

    print("\n[Telemetry 처리 중...]")
    X_st, y_st, subj_st = process_dataset(
        TELEMETRY_PATH, channels, label='telemetry')
    ...
    np.save(os.path.join(PROCESSED_PATH,
            f'X_train_comb_{config_name}.npy'), X_tr_c)
    np.save(os.path.join(PROCESSED_PATH,
            f'y_train_comb_{config_name}.npy'), y_tr_c)
    np.save(os.path.join(PROCESSED_PATH,
            f'X_val_comb_{config_name}.npy'),   X_val_c)
    np.save(os.path.join(PROCESSED_PATH,
            f'y_val_comb_{config_name}.npy'),   y_val_c)
    np.save(os.path.join(PROCESSED_PATH,
            f'X_test_comb_{config_name}.npy'),  X_te_c)
    np.save(os.path.join(PROCESSED_PATH,
            f'y_test_comb_{config_name}.npy'),  y_te_c)
    print(f"Combined {config_name} 저장 완료!")
