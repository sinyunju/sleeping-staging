...
- **Macro F1-score**: 주 평가지표. 클래스 불균형(REM 7%)에서 모든 클래스를 동등하게 평가
- **Cohen's Kappa**: 임상 전문가 간 판독 일치도(κ=0.7~0.8)와 비교
- **Accuracy**: 참고 지표
...
- **Claude (Sonnet 4.6)**: 코드 작성 보조, 실험 결과 분석 토론
- **Google Gemini**: 보고서 문장 다듬기
- **ChatGPT (GPT-4o)**: 모델 아키텍처 다이어그램 생성
...
2. Kemp et al. (1992). A simple format for exchange of digitized polygraphic recordings. *EEG & Clinical Neurophysiology*, 82(5).
3. Berry et al. (2012). AASM Scoring Manual. American Academy of Sleep Medicine.
4. Breiman (2001). Random forests. *Machine Learning*, 45(1).
5. He et al. (2016). Deep residual learning for image recognition. *CVPR*.
6. Hochreiter & Schmidhuber (1997). Long short-term memory. *Neural Computation*, 9(8).
7. Vaswani et al. (2017). Attention is all you need. *NeurIPS*.
8. Bergstra & Bengio (2012). Random search for hyper-parameter optimization. *JMLR*, 13.
EOF
echo "README 생성 완료"
